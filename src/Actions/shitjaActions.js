...
export const getAllGamesActionHandler = (currentGame) => {
    return dispatch => {
      ...
      axios.get('/ndeshjet/', {headers:{Authorization: "Token " + token}})
        .then(res => {
          // console.log("LOJA E MOMENTIT", currentGame)

              axios.get('/shitjet?ndeshja=' + currentGame, {headers:{Authorization: "Token " + token}})
              .then(shitjaJson => {
  ...
  const getAllGamesSuccess = (data, shitjaJson) => ({
    ...
    payload: {"allGames": data, "shitjaJson": shitjaJson.data}
  });
