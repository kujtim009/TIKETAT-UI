...
export const getCurrentGame = () => {
  return dispatch => {
    ...
    axios.get('/ndeshjetcurrent', {headers:{Authorization: "Token " + token}})
      .then(res => {    

                axios.get('/llojiindeshjeve', {headers:{Authorization: "Token " + token}})
              .then(ndeshjetRes => {
                localStorage.setItem('currentGameID', res.data.id)
                dispatch(getGameSuccess({...res.data, llojetNdeshjet: ndeshjetRes.data}));
              })
              .catch(err => {
...
export const saveNewGameActionHandler = (data) => {
  return dispatch => {
    ...
    const token = localStorage.getItem('token');
    axios.post('/ndeshjet/', data, {headers:{Authorization: "Token " + token}})
      .then(res => {
        localStorage.setItem('currentGameID', res.data.id)
...
export const newGameActionHandler = () => {
  return dispatch => {
    ...
    const token = localStorage.getItem('token');
    axios.get('/ulsetupdateall/', {headers:{Authorization: "Token " + token}})
      .then(res => {
...
export const getPricesActionHandler = () => {
  return dispatch => {
    ...
    const token = localStorage.getItem('token');
    axios.get('/cmimetgroup', {headers:{Authorization: "Token " + token}})
      .then(res => {
...
export const updatePriceActionHandler = (data) => {
  return dispatch => {
    ...
    const token = localStorage.getItem('token');
    axios.post('/cmimetgroup/', data, {headers:{Authorization: "Token " + token}})
      .then(res => {
